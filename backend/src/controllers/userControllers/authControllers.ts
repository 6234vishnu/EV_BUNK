...
export const createUser = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    if (enterdOtp !== otpFromBackend) {
      return res.status(200).json({ success: false, message: 'Incorrect OTP. Try again.' });
    }

    const passwordHash = await hashPassword(formdata.password);
    if (!passwordHash) {
      return res.status(200).json({ success: false, message: 'Server error, try later' });
    }
    ...
    if (!savedUser) {
      return res.status(200).json({ success: false, message: 'Couldn’t sign up, try again later' });
    }
...
export const setOtpForgotPassword=async(req:Request,res:Response):Promise<any>=>{
  try {
    const {email}=req.body
    const findUser=await User.findOne({email})
    if(!findUser){
      return res.status(200).json({ success: false, message: 'Couldint find user in this email' });
    }
    const otp = Math.floor(100000 + Math.random() * 900000).toString(); // 6 digit OTP
    console.log('otp: ',otp);
...
export const ResetPassword=async(req:Request,res:Response):Promise<any>=>{
  ...
  try {
    ...
    const findUser=await User.findOne({email})
    if(!findUser) return res.status(200).json({ success: false, message: 'Couldint find user in this email' });
    const passwordHash=await hashPassword(newPassword)
    if(!passwordHash)return res.status(200).json({ success: false, message: 'Server error try later' });
    const updateUser=await User.findByIdAndUpdate(findUser._id,{$set:{password:passwordHash}},{new:true})
    if(!updateUser)return res.status(200).json({ success: false, message: 'Server error try later' });
