...
const AdminLoginPage: React.FC = () => {
  ...
  const [forgotPasswordModal,setForgorPasswordModal]=useState<boolean>(false)
  ...
  const handleSubmit =async (e: React.FormEvent) => {
    e.preventDefault();
    if(!email&&!password) return setMessage("Enter Email and Password to login")
        try {
  ...
  const newPassword=async()=>{
    ...
    if(!password&&!confirmPassword ) setMessage('Enter password and confirm password')
    if(password!==confirmPassword ) setMessage('Password are not matched')
        try {
            const response=await api.post('/admin/auth/newPassword',{email,password})
            if(response.data.success){
                localStorage.setItem("adminId",response.data.adminId)
                navigate("/admin/dashboard")
            }
  ...
  return (
        ...
        <div className="adminLoginPage-content">
          ...
          <div className="adminLoginPage-formSide">
            ...
            <div className="adminLoginPage-formContainer">
              ...
              <h6>{message}</h6>
              <p className="adminLoginPage-subtitle">Enter your credentials to access the dashboard</p>
              ...
              <form onSubmit={handleSubmit} className="adminLoginPage-form">
                ...
                <div className="adminLoginPage-options">
                  ...
                  <a onClick={()=>setForgorPasswordModal(true)} href="#" className="adminLoginPage-forgotPassword">Forgot password?</a>
                </div>
    ...
    {forgotPasswordModal &&(
         ...
         <div className="adminLoginPage-forgotModal">
           ...
           <p style={{color:"black"}}>Please enter your email to reset your password.</p>
           ...
           <button className="adminLoginPage-closeBtn" onClick={()=>setForgorPasswordModal(false)}>
             Close
           </button>
         </div>
       </div>
    ...
    {enterOtpModal&&(
         ...
         <div className="otpModal-container">
           ...
           <div className="otpModal-buttons">
             ...
             <button onClick={()=>setEnterOtpModal(false)} className="otpModal-closeBtn">
               Close
             </button>
         
           </div>
            <div style={{padding:"20px"}}>
                <h6 style={{color:"black"}}>Time Remaining: <strong style={{color:"red"}}>{timer}s</strong></h6>
                {timerStopped && (
  <a style={{color:"black"}} href="#" onClick={(e) => {
    e.preventDefault();
    ...
    {newPasswordModal&&(
        ...
        <div className="newPasswordModal-container">
          ...
          <div className="newPasswordModal-buttons">
            <button onClick={newPassword} className="newPasswordModal-submitBtn">
              Submit
            </button>
            <button onClick={()=>setNewpasswordModal(false)} className="newPasswordModal-closeBtn">
              Close
            </button>
          </div>
        </div>
