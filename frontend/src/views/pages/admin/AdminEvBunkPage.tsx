...
const AdminEvBunkPage: React.FC = () => {
  ...
  const handleInputChange = (
    ...
    if (name === "supportedConnectors") {
      setFormData((prevData) => ({
        ...prevData,
        [name]: value
          ? value.split(",").map((connector) => connector.trim())
          : [],
      }));
      ...
      setFormData((prevData) => ({
        ...prevData,
        [name]: value
          ? value.split(",").map((landmark) => landmark.trim())
          : [],
      }));
    } else {
      setFormData((prevData) => ({
  ...
  const validateForm = (): ValidationResult => {
    ...
    if (!formData.latitude?.trim()) {
      errors.latitude = "Please provide latitude to conitnue";
    }
    if (!formData.longitude?.trim()) {
      errors.latitude = "Please provide longitude to conitnue";
    }
    ...
    if (
      ...
      errors.availablePorts =
        "Available Ports must be between 0 and Total Ports";
      isValid = false;
    }
    ...
    if (
      !formData.supportedConnectors ||
      formData.supportedConnectors.length === 0
    ) {
      errors.supportedConnectors =
        "At least one supported connector is required";
      isValid = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Convert Latitude
    const latitudeDMS = formData.latitude.trim(); // Example: "11°48'00.5\"N"
    const [latDeg, latMin, latSec] = latitudeDMS
      .split(/[^\d.]+/)
      .filter(Boolean)
      .map(Number);
    const latDirection = latitudeDMS.slice(-1);
    const decimalLatitude = convertDMSToDecimal(
      latDeg,
      latMin,
      latSec,
      latDirection
    ...
    // Convert Longitude
    const longitudeDMS = formData.longitude.trim(); // Example: "75°59'42.9\"E"
    const [longDeg, longMin, longSec] = longitudeDMS
      .split(/[^\d.]+/)
      .filter(Boolean)
      .map(Number);
    const longDirection = longitudeDMS.slice(-1);
      ...
      >
                ...
                <form className="adminEvBunkPage__form" onSubmit={handleSubmit}>
                  <div className="adminEvBunkPage__form-grid">
                    ...
                    <div className="adminEvBunkPage__form-group">
                      <label
                        htmlFor="name"
                        className="adminEvBunkPage__form-label"
                      >
                        Bunk Name{" "}
                        <span className="adminEvBunkPage__required">*</span>
                      </label>
                      <input
                        type="text"
                        id="name"
                      ...
                      />
                      {formErrors.name && (
                        <div className="adminEvBunkPage__error-message">
                          {formErrors.name}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="adminEvBunkPage__form-group adminEvBunkPage__form-group--full">
                      ...
                      <div className="adminEvBunkPage__map-help">
                        <small>
                          Tip: Go to Google Maps, search for the location, click
                          "Share", then "Embed a map" and copy the src URL from
                          the iframe code.
                        </small>
                      </div>
                    </div>
                    ...
                    <div className="adminEvBunkPage__form-group">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className={`adminEvBunkPage__form-input ${
                          formErrors.supportedConnectors
                            ? "adminEvBunkPage__form-input--error"
                            : ""
                        }`}
                    ...
                    <div className="adminEvBunkPage__form-group adminEvBunkPage__form-group--full">
                      ...
                      <button
                        style={{ color: "white" }}
                        type="submit"
                        className="adminEvBunkPage__form-submit-button"
                      >
                        Submit
                      </button>
                    </div>
        ...
        <div className="adminEvBunkPage__footer">
          <p className="adminEvBunkPage__copyright">
            © {new Date().getFullYear()} EV Admin Dashboard. All rights
            reserved.
          </p>
        </div>
      </div>
