...
function LoginPage() {
  ...
  const submitNewPassword = async () => {
    if (!newPassword && !confirmPassword)
      return setMessage("fill all the feilds ");

    if (newPassword !== confirmPassword)
      return setMessage("passwords didint match");

    try {
      const response = await api.post("/user/auth/newPassword", {
        newPassword,
        email,
      });
      if (response.data.success) {
        ...
        {!showLoginForm ? (
          ...
          <div className="loginFormContainer">
            ...
            <input
              ...
              required
            />
            <button onClick={userLogin} className="loginButton">
              Login
            </button>
        ...
        <div className="forgotModalOverlay">
          <div className="forgotModal">
            ...
            <h6 style={{ color: "black" }}>
              NOTE: <strong>Check Your email</strong>
            </h6>
            <div className="forgotModalButtons">
              <button onClick={handleForgotSubmit} className="loginButton">
                Get Otp
              </button>
              <button
                onClick={() => setShowForgotModal(false)}
                className="loginButton cancel"
              >
                Close
              </button>
            </div>
